...
public interface AllLeaderboardsApi {
    ...
    @Operation(operationId = "escapeRoomSessionsGet", summary = "Get all session leaderboards", description = "Retrieve leaderboards for all sessions, optionally filtered by session tags.", tags = {
            "all-leaderboards" }, responses = {
                    @ApiResponse(responseCode = "200", description = "All session leaderboards", content = {
                            @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = FullExportResponse.class))) }),
    ...
    default ResponseEntity<List<FullExportResponse>> escapeRoomSessionsGet(
            @Parameter(name = "tag", description = "A tags to filter sessions.", in = ParameterIn.QUERY) @Valid @RequestParam(value = "tag", required = false) String tag) {
        return getDelegate().escapeRoomSessionsGet(tag);
    }
