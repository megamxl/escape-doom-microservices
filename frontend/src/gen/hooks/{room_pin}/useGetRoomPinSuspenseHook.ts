...
export const getRoomPinSuspenseQueryKey = ({ room_pin }: { room_pin: GetRoomPinPathParams['room_pin'] }) =>
  ['v5', { url: '/:room_pin', params: { room_pin: room_pin } }] as const
...
export async function getRoomPinSuspenseHook(
  ...
  const res = await request<GetRoomPinQueryResponse, ResponseErrorConfig<GetRoomPin404>, unknown>({ method: 'GET', url: `/${room_pin}`, ...requestConfig })
  return res.data
}
