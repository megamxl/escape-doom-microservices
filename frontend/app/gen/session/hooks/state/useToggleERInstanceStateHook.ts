...
export const toggleERInstanceStateMutationKey = () => [{ url: '/state/{session_id}/{state}' }] as const
 ...
 * {@link /state/:session_id/:state}
 */
export async function toggleERInstanceStateHook(
  { session_id, state }: { session_id: ToggleERInstanceStatePathParams['session_id']; state: ToggleERInstanceStatePathParams['state'] },
...
export function useToggleERInstanceStateHook(
  options: {
    mutation?: UseMutationOptions<
      ...
      { session_id: ToggleERInstanceStatePathParams['session_id']; state: ToggleERInstanceStatePathParams['state'] }
    >
  ...
  return useMutation<
    ...
    { session_id: ToggleERInstanceStatePathParams['session_id']; state: ToggleERInstanceStatePathParams['state'] }
  >({
    mutationFn: async ({ session_id, state }) => {
      return toggleERInstanceStateHook({ session_id, state }, config)
    },
    mutationKey,
    ...mutationOptions,
  })
